...
# Xóa một sinh viên (biết chính xác cả điểm và tên)
danh_sach_sv.remove((7.0, "Binh"))

# Sửa điểm cho sinh viên ở vị trí xác định (ví dụ vị trí 0)
danh_sach_sv[0] = (9.0, danh_sach_sv[0][1])

# Kiểm tra một sinh viên có trong danh sách hay không (dùng toán tử in)
print("Chi co trong danh sach khong?", (9.2, "Chi") in danh_sach_sv)

# Sắp xếp theo điểm tăng dần (mặc định so sánh phần tử đầu tiên của tuple trước)
danh_sach_sv.sort()
